...
import { OrderPlacedPayloadSchema, Order } from "../services/types";
import { OrderBook } from "../services/order-book";

export const config: EventConfig = {
    type: 'event',
    name: 'PublishOrderbookFeed',
    description: 'Publish orderbook snapshots to real-time stream',
...
export const handler: Handlers['PublishOrderbookFeed'] = async (input, { state, streams, logger }) => {
    try {
...
async function publishOrderbookSnapshot(instrument: string, state: any, streams: any, logger: any) {
    ...
    logger.info('Orderbook published to stream', { instrument, bids: bids.length, asks: asks.length });
}
