...
declare module 'motia' {
  ...
  interface Handlers {
    ...
    'ProcessCancellation': EventHandler<{ orderId: string; instrument: 'AAPL'; side: 'BUY' | 'SELL' }, { topic: 'cancellation.recorded'; data: { orderId: string; instrument: string; side: string } }>
    'PlaceOrder': ApiRouteHandler<{ userId: string; side: 'BUY' | 'SELL'; type: 'LIMIT' | 'MARKET'; instrument: 'AAPL'; price?: number; quantity: number }, ApiResponse<201, { orderId: string }> | ApiResponse<400, { error: string }>, { topic: 'order.placed'; data: { orderId: string; instrument: string } }>
